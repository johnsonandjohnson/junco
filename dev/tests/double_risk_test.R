...
rr_splitfun <- make_multicomp_splfun(colspan_trt_map)

lyt <- basic_table() |>
    split_cols_by("colspan_trt", split_fun = trim_levels_to_map(colspan_trt_map)) |>
    split_cols_by(trtvar, show_colcounts = TRUE) |>
    split_cols_by("rrisk_header", nested = FALSE) |>
    split_cols_by(trtvar, split_fun = rr_splitfun) |>
    analyze("AESDTH", afun = function(x, ...) rcell("-"))

build_table(lyt, adae, adsl)

rr_splitfun2 <- make_multicomp_splfun(colspan_trt_map, combo_levels_map = combodf, comp_level_map = comp_map)
...
lyt <- basic_table() |>
    ...
    split_cols_by("rrisk_header", nested = FALSE) |>
    split_cols_by(trtvar, split_fun = rr_splitfun2) |>
...
afun_show_path <- function(x, ref_path = NULL, ...) rcell(paste(ref_path, collapse = "."))

lyt <- basic_table() |>
    col_struct_w_risk_diffs(colspan_trt_map,
                            combo_map_df = NULL,
                            comp_map = NULL) |>
    analyze("AESDTH", afun = afun_show_path)

tbl <- build_table(lyt, adae, adsl)


rr_splitfun <- make_multicomp_splfun2(colspan_trt_map, combodf, NULL)
...
rr_splitfun2 <- make_multicomp_splfun2(colspan_trt_map, combodf, data.frame(active = c("all_patients", "all_active"), comparator = c("Placebo", "Std Of Care")))
