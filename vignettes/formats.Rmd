...
for an overview of the built-in string formats. 

Another approach for 
format specification is via formatting functions. With formatting functions, 
...
We will start with demonstration of various basic approaches for writing an `afun` 
...
is_str_style <- sapply(junco_fmt_numeric_vars, function(x) {
  !is.function(x)
})
...
test <- adsl %>%
  mutate(num_var = case_when(
    ARMCD == "ARM A" & SEX == "F" ~ 3.75,
...
tbl <- basic_table(show_colcounts = TRUE) %>%
  ...
  analyze(
    vars = "num_var", afun = tern::a_summary,
    extra_args = list(
      .stats = c("n", "mean"),
      .formats = c(mean = "xx.x")
    )
...
was useful for understanding how table tree object get printed.
...
and what gets processe later (typically exporting to other format, like rtf, html).
