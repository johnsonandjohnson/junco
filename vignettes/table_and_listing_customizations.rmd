...
If a .docx file is to be created for the table, the `junco::export_TLG_as_docx` is to be called. For .docx files, the markup is different from .rtf files. In this example, when calling `junco::export_TLG_as_docx`, the "markup_df" argument has the default "docx_markup_df" object specified.
...
tt_to_flextable_j(result, "superscripttable", orientation = "landscape",  markup_df = docx_markup_df)
```
...
tt_to_flextable_j(result, "examplelistingsuperscript2", orientation = "landscape",  markup_df = docx_markup_df)
```
