...
tt_to_tlgrtf(result,
             file = paste0(getwd(), "/exampleorientation"),
...
```{r orientation_docx_display, echo=FALSE, fig.align="center", out.width="100%", out.height="200px"}
tt_to_flextable_j(result, "exampleorientation", orientation = "landscape")
```
...
To export to docx, the `junco::export_as_docx_j` function can be called similarly to `junco::tt_to_tlgrtf`
...
```{r fontsize_docx_display, echo=FALSE, fig.align="center", out.width="100%", out.height="200px"}
tt_to_flextable_j(result, "examplefontsize", theme = theme_docx_default_j(font = "Times New Roman", font_size = 8L))
```
...
```{r combined_column_setup, eval=FALSE}
add_combo <- add_combo_facet("Combined",
    label = "Combined",
    levels = c("Example Drug 5 mg", "Example Drug 10 mg", "Example Drug 20 mg")
  )
...
The default markup file for `junco::export_as_docx_j` to produce an .rtf file is as follows.
TBD
...
### Listing .docx File
The `junco::tt_to_tlgdocx` function can be called multiple times to produce multiple .docx files.
TBD
