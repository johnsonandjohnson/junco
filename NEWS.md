...
- Moved `safe_t_test()` to a new file `safe_t_test.R`,
  and `add_blank_line_rcells()` from `s_functions.R` to `blank_line.R`.
...
- Hotfix: Added several new functions for creating the Vital Sign tables for core and clinpharm:
  ...
  `prepend_label_cell()`, `s_diff_mean_ci()`, `s_summary_diff()`, `safe_t_test()`.
