...
- section_properties:
  ...
  size, orientation, margins, etc. See officer::prop_section() for more
  details. No need to be specified by end user.
...
- template_file:
  ...
  (optional). Default = "doc/template_file.docx". Paragraph styles are
...
- combined_docx:
  ...
  (optional). Default = FALSE. Whether to also export an "allparts" docx
  version.
...
- pagenum:
  ...
  (optional). Whether to display page numbers. Only applicable to
  listings (i.e. for tables and figures this argument is ignored).
...
- watermark:
  ...
  (`logical`)  
  whether to display the watermark "Confidential". By default, this is
  set to FALSE. In the future, this argument will be the actual
  watermark (i.e. a string) to display.
