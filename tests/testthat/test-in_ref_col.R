test_that("in_ref_col returns FALSE when ref_path is NULL regardless of .spl_context", {
  spl_context <- data.frame(
    cur_col_split = I(list("ARM")),
    cur_col_split_val = I(list("Placebo"))
  )

  expect_identical(in_ref_col(NULL, spl_context), FALSE)
  expect_identical(in_ref_col(NULL, data.frame()), FALSE)
})
